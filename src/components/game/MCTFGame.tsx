...
export default function MCTFGame({ game }: Props) {
  ...
  if (hasEnded) {
    return <GameCompletion gameId={ game.id } totalCorrect={ correctAnswers } questionAmount={ game.questions.length } timeCompleted={formatTimeDelta(differenceInSeconds(now, game.timeStarted))} />
  }
  ...
  return (
    <div className="flex flex-col items-center justify-between max-w-2xl w-full mx-auto m-4 md:mt-8 px-4">
      ...
      <div className="w-full p-6 bg-white border-2 border-rose-100 rounded-lg shadow shadow-rose-50">
        ...
        <button 
            className="flex gap-2 items-center justify-between mt-5 bg-rose-400 text-white font-semibold text-xl p-2 rounded-lg mx-auto hover:bg-rose-500 disabled:bg-gray-400 disabled:hover:bg-gray-400" 
